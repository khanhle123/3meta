...
exports.default = translateChart;
...
function tr(category, val, locale) {
    var _a;
    if (val === undefined || val === null)
        return val;
    if (Array.isArray(val))
        return val.map((v) => tr(category, v, locale));
    if (typeof val !== 'string')
        return val;
    const key = `${category}.${val}`;
    const result = index_1.i18n.t(key);
    if (result !== key)
        return result; // dịch thành công
    const fallback = (_a = NONE_FALLBACK[locale]) === null || _a === void 0 ? void 0 : _a[val];
    return fallback !== null && fallback !== void 0 ? fallback : val; // không tìm thấy -> giữ nguyên giá trị gốc
}
function translateGanZhi(gz, locale) {
    if (!gz || gz.length < 2)
        return gz;
...
function translateTimeInfo(t, locale) {
    if (!t)
        return t;
    return {
        ...t,
        ...
        voidness: Array.isArray(t.voidness) ? t.voidness.map((b) => tr('branches', b, locale)) : t.voidness,
...
function translateFourPillars(fp, locale) {
    if (!fp)
        return fp;
    const one = (pillar) => pillar ? { stem: tr('stems', pillar.stem, locale), branch: tr('branches', pillar.branch, locale) } : pillar;
    return { year: one(fp.year), month: one(fp.month), day: one(fp.day), hour: one(fp.hour) };
}
function translateVoidness(v, locale) {
    if (!v)
        return v;
    return {
        ...v,
        voidBranches: Array.isArray(v.voidBranches) ? v.voidBranches.map((b) => tr('branches', b, locale)) : v.voidBranches,
        palaceBranches: Array.isArray(v.palaceBranches)
            ? v.palaceBranches.map((b) => { var _a, _b; return (b === '无' ? (_b = (_a = NONE_FALLBACK[locale]) === null || _a === void 0 ? void 0 : _a['无']) !== null && _b !== void 0 ? _b : b : tr('branches', b, locale)); })
            : v.palaceBranches,
        voidInPalace: Array.isArray(v.voidInPalace) ? v.voidInPalace.map((b) => tr('branches', b, locale)) : v.voidInPalace,
    };
}
function translateGrowthInfo(g, locale) {
    if (!g)
        return g;
    const one = (v) => {
        var _a, _b;
        if (Array.isArray(v))
            return v.map(one);
        if (v === '无')
            return (_b = (_a = NONE_FALLBACK[locale]) === null || _a === void 0 ? void 0 : _a['无']) !== null && _b !== void 0 ? _b : v;
...
function translateTombInfo(t, locale) {
    if (!t)
        return t;
    return {
        ...t,
        heavenlyStemInTomb: Array.isArray(t.heavenlyStemInTomb) ? t.heavenlyStemInTomb.map((s) => tr('stems', s, locale)) : t.heavenlyStemInTomb,
        earthlyStemInTomb: Array.isArray(t.earthlyStemInTomb) ? t.earthlyStemInTomb.map((s) => tr('stems', s, locale)) : t.earthlyStemInTomb,
        tombBranch: t.tombBranch ? tr('branches', t.tombBranch, locale) : t.tombBranch,
    };
}
function translateRelationEntry(e, locale) {
    if (!e)
        return e;
    ...
    if (e.id && e.params) {
        const p = e.params;
        description = index_1.i18n.t(e.id, {
            ...p,
...
function translateTenStemResponse(r, locale) {
    if (!r)
        return r;
    return {
...
function translatePattern(pat) {
    ...
    return {
        ...
        description: (0, formatters_1.formatPattern)(pat),
    };
}
function translatePalace(p, locale) {
    var _a, _b, _c, _d, _e, _f;
    if (!p)
        return p;
    const translatePatternEntry = (pat) => {
    ...
    return {
        ...
        earthBranch: p.earthBranch === '无' ? (_b = (_a = NONE_FALLBACK[locale]) === null || _a === void 0 ? void 0 : _a['无']) !== null && _b !== void 0 ? _b : p.earthBranch : tr('branches', p.earthBranch, locale),
        voidness: translateVoidness(p.voidness, locale),
        fiveElements: tr('fiveElements', p.fiveElements, locale),
        status: p.status
            ? {
                star: p.status.star === '无' ? (_d = (_c = NONE_FALLBACK[locale]) === null || _c === void 0 ? void 0 : _c['无']) !== null && _d !== void 0 ? _d : p.status.star : tr('status.star', p.status.star, locale),
                gate: p.status.gate === '无' ? (_f = (_e = NONE_FALLBACK[locale]) === null || _e === void 0 ? void 0 : _e['无']) !== null && _f !== void 0 ? _f : p.status.gate : tr('status.gate', p.status.gate, locale),
            }
            : p.status,
...
function translateSpecialPatterns(sp, locale) {
    if (!sp)
        return sp;
    return {
        ...sp,
        menPo: Array.isArray(sp.menPo)
            ? sp.menPo.map((m) => {
                ...
                if (m.id) {
                    const p = m.params || {};
                    description = index_1.i18n.t(m.id, {
                        ...p,
 ...
 */
function translateChart(chart, locale) {
    const previousLocale = index_1.i18n.getLocale();
    index_1.i18n.setLocale(locale);
    try {
        return {
            ...
            palaces: Array.isArray(chart.palaces) ? chart.palaces.map((p) => translatePalace(p, locale)) : chart.palaces,
            hiddenStems: chart.hiddenStems instanceof Map
                ? Object.fromEntries(Array.from(chart.hiddenStems.entries()).map(([k, v]) => [k, tr('stems', v, locale)]))
                : chart.hiddenStems,
            specialPatterns: translateSpecialPatterns(chart.specialPatterns, locale),
        };
    }
    finally {
        index_1.i18n.setLocale(previousLocale); // không làm rò rỉ locale global ra ngoài phạm vi hàm
    }
}
