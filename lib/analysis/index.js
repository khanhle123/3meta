...
exports.calcMenPo = exports.detectGlobalPatterns = exports.detectPatterns = exports.calcStatus = exports.calcGatePressure = exports.calcTenStemResponse = exports.calcTombInfo = exports.calcGrowth = exports.getPostHorse = exports.buildVoidness = exports.getInnerOuter = exports.getMonthElementByBranch = exports.getSeasonByMonthBranch = void 0;
...
const getSeasonByMonthBranch = (branch) => {
    // 季节按月建（节气历）月支的五行判定：寅卯木=春, 巳午火=夏, 申酉金=秋, 亥子水=冬, 辰戌丑未土=四季月。
    // 注意：不能用「子=正月」的序号映射，月建正月建寅，子月为十一月（冬）。
    const element = constants_1.BRANCH_FIVE_ELEMENTS[branch];
    if (element === '木')
        return '春';
    if (element === '火')
        return '夏';
...
exports.getSeasonByMonthBranch = getSeasonByMonthBranch;
const getMonthElementByBranch = (branch) => {
    return constants_1.BRANCH_FIVE_ELEMENTS[branch];
};
exports.getMonthElementByBranch = getMonthElementByBranch;
const getInnerOuter = (isYangdun, position) => {
    if (position === 5)
        return '无';
    const yangInner = [1, 8, 3, 4];
    const yinInner = [9, 2, 7, 6];
    const inner = isYangdun ? yangInner : yinInner;
    return inner.includes(position) ? '内盘' : '外盘';
};
exports.getInnerOuter = getInnerOuter;
const buildVoidness = (xunShou, palaceBranches) => {
    const voidBranches = constants_1.XUN_SHOU_VOIDNESS[xunShou];
    const palaceArr = palaceBranches === '无' ? ['无'] : Array.isArray(palaceBranches) ? palaceBranches : [palaceBranches];
    const voidInPalace = palaceArr.filter((b) => voidBranches.includes(b));
    return {
        voidBranches,
...
exports.buildVoidness = buildVoidness;
const getPostHorse = (timeBranch) => {
    const key = Object.keys(constants_1.POST_HORSE_MAP).find((k) => k.includes(timeBranch)) || '申子辰';
    const branch = constants_1.POST_HORSE_MAP[key];
    return {
        branch,
        position: (0, palace_1.findPalaceByBranch)(branch) || 5,
    };
};
exports.getPostHorse = getPostHorse;
const resolveGrowthForBranch = (stem, branch) => {
...
const calcGrowth = (stems, palaceBranches) => {
    if (stems === '无' || palaceBranches === '无')
        return '无';
...
const calcTombInfo = (heavenlyStems, earthlyStems, palaceBranches, timeStem, dayStem) => {
    ...
    const allStems = [];
    if (heavenlyStems !== '无') {
        if (Array.isArray(heavenlyStems))
            allStems.push(...heavenlyStems);
        else
            allStems.push(heavenlyStems);
    }
    if (earthlyStems !== '无') {
        if (Array.isArray(earthlyStems))
            allStems.push(...earthlyStems);
        else
            allStems.push(earthlyStems);
    }
    if (timeStem)
        allStems.push(timeStem);
    if (dayStem)
        allStems.push(dayStem);
    return {
        heavenlyStemInTomb,
        earthlyStemInTomb,
...
exports.calcTombInfo = calcTombInfo;
const relate = (src, target) => {
    if (src === target)
        return '比和';
    if (GENERATE[src] === target)
        return '我生';
    if (GENERATE[target] === src)
        return '生我';
    if (OVERCOME[src] === target)
        return '我克';
    if (OVERCOME[target] === src)
        return '克我';
    return '无';
};
const calcTenStemResponse = (heavenly, earthly, timeStem, dayStem) => {
    ...
    const heavenlyToEarthly = (!h || !e)
        ...
        : (() => {
            const rel = relate(constants_1.STEM_FIVE_ELEMENTS[h], constants_1.STEM_FIVE_ELEMENTS[e]);
            return {
                relation: rel,
                id: 'patterns.ten_stem.template',
                params: { h, hEle: constants_1.STEM_FIVE_ELEMENTS[h], rel, e, eEle: constants_1.STEM_FIVE_ELEMENTS[e] },
                description: `${h}${constants_1.STEM_FIVE_ELEMENTS[h]}${rel === '比和' ? '比和' : rel === '我生' ? '生' : rel === '生我' ? '被生' : rel === '我克' ? '克' : '被克'}${e}${constants_1.STEM_FIVE_ELEMENTS[e]}`,
                isAuspicious: ['比和', '生我', '我生'].includes(rel)
            };
        })();
    const build = (src, target) => {
        if (!src || !target)
            return { relation: '无', description: '无' };
        const rel = relate(constants_1.STEM_FIVE_ELEMENTS[src], constants_1.STEM_FIVE_ELEMENTS[target]);
        return {
            relation: rel,
            id: 'patterns.ten_stem.template',
            params: { h: src, hEle: constants_1.STEM_FIVE_ELEMENTS[src], rel, e: target, eEle: constants_1.STEM_FIVE_ELEMENTS[target] },
            description: `${src}${constants_1.STEM_FIVE_ELEMENTS[src]}${rel === '比和' ? '比和' : rel === '我生' ? '生' : rel === '生我' ? '被生' : rel === '我克' ? '克' : '被克'}${target}${constants_1.STEM_FIVE_ELEMENTS[target]}`,
            isAuspicious: ['比和', '生我', '我生'].includes(rel)
        };
    };
    return {
...
exports.calcTenStemResponse = calcTenStemResponse;
const calcGatePressure = (gate, palaceElement) => {
    if (gate === '无门')
        return '无';
    const gateEle = constants_1.GATE_FIVE_ELEMENTS[gate];
    if (OVERCOME[gateEle] === palaceElement)
        return '迫';
    if (OVERCOME[palaceElement] === gateEle)
        return '制';
    if (GENERATE[gateEle] === palaceElement)
        return '和';
    if (GENERATE[palaceElement] === gateEle)
        return '义';
    return '无';
};
exports.calcGatePressure = calcGatePressure;
const calcStatus = (star, gate, season) => {
    const s = Array.isArray(star) ? star[0] : star;
    const starStatus = (s === '天禽' || s === '无' || !STAR_STATUS_TABLE[s]) ? '无' : STAR_STATUS_TABLE[s][season];
    const gateStatus = gate === '无门' ? '无' : GATE_STATUS_TABLE[gate][season];
    return { star: starStatus, gate: gateStatus };
};
exports.calcStatus = calcStatus;
const detectPatterns = (palace, zhiShiGate, fourPillars) => {
    const auspicious = [];
    const inauspicious = [];
    ...
    if (sanMen) {
        if (earthly.includes('乙') && palace.position === 3)
            auspicious.push({ id: 'patterns.common.qi_you_lu_wei', name: '奇游禄位', type: '奇游禄位', position: palace.position, params: { earth: '乙', palace: '震3宫', gate: palace.gate }, description: '地盘乙+震3宫+开/休/生门' });
        if (earthly.includes('丙') && palace.position === 4)
            auspicious.push({ id: 'patterns.common.qi_you_lu_wei', name: '奇游禄位', type: '奇游禄位', position: palace.position, params: { earth: '丙', palace: '巽4宫', gate: palace.gate }, description: '地盘丙+巽4宫+开/休/生门' });
        if (earthly.includes('丁') && palace.position === 9)
            auspicious.push({ id: 'patterns.common.qi_you_lu_wei', name: '奇游禄位', type: '奇游禄位', position: palace.position, params: { earth: '丁', palace: '离9宫', gate: palace.gate }, description: '地盘丁+离9宫+开/休/生门' });
    }
    // 欢怡：地盘乙/丙/丁+值符
    ...
    // 门宫和义
    if (palace.gate !== '无门') {
        const gateEle = constants_1.GATE_FIVE_ELEMENTS[palace.gate];
    ...
    if (heavenly.includes('乙') && earthly.includes('辛'))
        inauspicious.push({ id: 'patterns.bad.qing_long_tao_zou', name: '青龙逃走', type: '青龙逃走', position: palace.position, params: { heaven: '乙', earth: '辛' }, description: '天盘乙被地盘辛克' });
    if (heavenly.includes('辛') && earthly.includes('乙'))
        inauspicious.push({ id: 'patterns.bad.bai_hu_chang_kuang', name: '白虎猖狂', type: '白虎猖狂', position: palace.position, params: { heaven: '辛', earth: '乙' }, description: '天盘辛克地盘乙' });
    if (heavenly.includes('丙') && earthly.includes('庚'))
        inauspicious.push({ id: 'patterns.bad.ying_ru_tai_bai', name: '荧入太白', type: '荧入太白', position: palace.position, params: { heaven: '丙', earth: '庚' }, description: '天盘丙克地盘庚' });
    if (heavenly.includes('庚') && earthly.includes('丙'))
        inauspicious.push({ id: 'patterns.bad.tai_bai_ru_ying', name: '太白入荧', type: '太白入荧', position: palace.position, params: { heaven: '庚', earth: '丙' }, description: '天盘庚克地盘丙' });
    // 朱雀投江：天盘丁+地盘癸
    if (heavenly.includes('丁') && earthly.includes('癸'))
        inauspicious.push({ id: 'patterns.bad.zhu_que_tou_jiang', name: '朱雀投江', type: '朱雀投江', position: palace.position, params: { heaven: '丁', earth: '癸' }, description: '天盘丁+地盘癸' });
    // 螣蛇夭矫：天盘癸+地盘丁
    if (heavenly.includes('癸') && earthly.includes('丁'))
        inauspicious.push({ id: 'patterns.bad.teng_she_yao_jiao', name: '螣蛇夭矫', type: '螣蛇夭矫', position: palace.position, params: { heaven: '癸', earth: '丁' }, description: '天盘癸+地盘丁' });
    // 大格：天盘庚+地盘癸
    if (heavenly.includes('庚') && earthly.includes('癸'))
        inauspicious.push({ id: 'patterns.bad.da_ge', name: '大格', type: '大格', position: palace.position, params: { heaven: '庚', earth: '癸' }, description: '天盘庚+地盘癸' });
    // 小格：天盘庚+地盘壬
    if (heavenly.includes('庚') && earthly.includes('壬'))
        inauspicious.push({ id: 'patterns.bad.xiao_ge', name: '小格', type: '小格', position: palace.position, params: { heaven: '庚', earth: '壬' }, description: '天盘庚+地盘壬' });
    // 刑格：天盘庚+地盘己
    if (heavenly.includes('庚') && earthly.includes('己'))
        inauspicious.push({ id: 'patterns.bad.xing_ge', name: '刑格', type: '刑格', position: palace.position, params: { heaven: '庚', earth: '己' }, description: '天盘庚+地盘己' });
    // 伏宫格：天盘庚+地盘戊
    if (heavenly.includes('庚') && earthly.includes('戊'))
        inauspicious.push({ id: 'patterns.bad.fu_gong_ge', name: '伏宫格', type: '伏宫格', position: palace.position, params: { heaven: '庚', earth: '戊' }, description: '天盘庚+地盘戊' });
    // 飞宫格：天盘戊+地盘庚
    if (heavenly.includes('戊') && earthly.includes('庚'))
        inauspicious.push({ id: 'patterns.bad.fei_gong_ge', name: '飞宫格', type: '飞宫格', position: palace.position, params: { heaven: '戊', earth: '庚' }, description: '天盘戊+地盘庚' });
    // 岁格：天盘庚+地盘年干
    if (heavenly.includes('庚') && earthly.includes(year.stem))
        inauspicious.push({ id: 'patterns.bad.sui_ge', name: '岁格', type: '岁格', position: palace.position, params: { heaven: '庚', earth: year.stem }, description: '天盘庚+地盘年干' });
    // 月格：天盘庚+地盘月干
    if (heavenly.includes('庚') && earthly.includes(month.stem))
        inauspicious.push({ id: 'patterns.bad.yue_ge', name: '月格', type: '月格', position: palace.position, params: { heaven: '庚', earth: month.stem }, description: '天盘庚+地盘月干' });
    // 日格：天盘庚+地盘日干
    ...
    // 天网四张：天盘癸+地盘癸
    if (heavenly.includes('癸') && earthly.includes('癸'))
        inauspicious.push({ id: 'patterns.bad.tian_wang_si_zhang', name: '天网四张', type: '天网四张', position: palace.position, params: { heaven: '癸', earth: '癸' }, description: '天盘癸+地盘癸' });
    // 悖格: 天盘丙+值符 or 地盘丙+值符
    if (heavenly.includes('丙') && palace.isZhiFu)
        inauspicious.push({ id: 'patterns.bad.bei_ge', name: '悖格', type: '悖格', position: palace.position, params: { position: '天盘', heaven: '丙' }, description: '天盘丙加值符' });
    if (earthly.includes('丙') && palace.isZhiFu)
        inauspicious.push({ id: 'patterns.bad.bei_ge', name: '悖格', type: '悖格', position: palace.position, params: { position: '地盘', heaven: '丙' }, description: '地盘丙加值符' });
...
const detectGlobalPatterns = (fourPillars) => {
    const auspicious = [];
    const inauspicious = [];
    ...
    if (['甲', '己'].includes(dayStem) && (timeGZ === '甲子' || timeGZ === '甲戌'))
        isTianXian = true;
    if (['乙', '庚'].includes(dayStem) && timeGZ === '甲申')
        isTianXian = true;
    if (['丙', '辛'].includes(dayStem) && timeGZ === '甲午')
        isTianXian = true;
    if (['丁', '壬'].includes(dayStem) && timeGZ === '甲辰')
        isTianXian = true;
    if (['戊', '癸'].includes(dayStem) && timeGZ === '甲寅')
        isTianXian = true;
    if (isTianXian) {
        auspicious.push({
    ...
    if (dayStem === '甲' && timeGZ === '庚午')
        isWuBuYuShi = true;
    if (dayStem === '乙' && timeGZ === '辛巳')
        isWuBuYuShi = true;
    if (dayStem === '丙' && timeGZ === '壬辰')
        isWuBuYuShi = true;
    if (dayStem === '丁' && timeGZ === '癸卯')
        isWuBuYuShi = true;
    if (dayStem === '戊' && timeGZ === '甲寅')
        isWuBuYuShi = true;
    if (dayStem === '己' && timeGZ === '乙丑')
        isWuBuYuShi = true;
    if (dayStem === '庚' && timeGZ === '丙子')
        isWuBuYuShi = true;
    if (dayStem === '辛' && timeGZ === '丁酉')
        isWuBuYuShi = true;
    if (dayStem === '壬' && timeGZ === '戊申')
        isWuBuYuShi = true;
    if (dayStem === '癸' && timeGZ === '己未')
        isWuBuYuShi = true;
    if (isWuBuYuShi) {
        inauspicious.push({
...
exports.detectGlobalPatterns = detectGlobalPatterns;
const calcMenPo = (palace, gatePressure) => {
    if (gatePressure !== '迫' && gatePressure !== '制')
        return undefined;
